...
func New(size, sectorSize int64, dir string, backingFile *backingfile.BackingFile, disableRevCounter, unmapMarkDiskChainRemoved bool, maximumSnapshotCount int, maximumTotalSnapshotSize int64) (*Replica, error) {
	return construct(false, size, sectorSize, dir, "", backingFile, disableRevCounter, unmapMarkDiskChainRemoved, maximumSnapshotCount, maximumTotalSnapshotSize)
}
...
func NewReadOnly(dir, head string, backingFile *backingfile.BackingFile) (*Replica, error) {
	...
	// maximumSnapshotCount and maximumTotalSnapshotSize don't matter because readonly replica can't create a new disk
	return construct(true, 0, diskutil.ReplicaSectorSize, dir, head, backingFile, false, false, 250, 0)
}

func construct(readonly bool, size, sectorSize int64, dir, head string, backingFile *backingfile.BackingFile, disableRevCounter, unmapMarkDiskChainRemoved bool, maximumSnapshotCount int, maximumTotalSnapshotSize int64) (*Replica, error) {
...
func (r *Replica) Reload() (*Replica, error) {
	newReplica, err := New(r.info.Size, r.info.SectorSize, r.dir, r.info.BackingFile, r.revisionCounterDisabled, r.unmapMarkDiskChainRemoved, r.maximumSnapshotCount, r.maximumTotalSnapshotSize)
	if err != nil {
		return nil, err
	}
...
func (r *Replica) SetMaximumSnapshotCount(count int) {
	r.Lock()
	defer r.Unlock()

	r.maximumSnapshotCount = count

	logrus.Infof("Set replica flag maximumSnapshotCount to %d", count)
}

func (r *Replica) SetMaximumTotalSnapshotSize(size int64) {
	r.Lock()
	defer r.Unlock()

	r.maximumTotalSnapshotSize = size

	logrus.Infof("Set replica flag maximumTotalSnapshotSize to %d", size)
}
